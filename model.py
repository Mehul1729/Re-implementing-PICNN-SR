...
class USCNNSep(nn.Module):
	def __init__(self,h,nx,ny,nVarIn=1,nVarOut=1,initWay=None,k=5,s=1,p=2):
		...
		Define net
		"""
		# inlet velocity as trainable param.
		...
		W2=32
		self.relu=nn.ReLU()
		self.US=nn.Upsample(size=[self.ny-2,self.nx-2],mode='bicubic')  # bicubic interpolation (Upscaled image is given as input to CNN)
  ...
  # Conv Layers:
		...
		#Specify filters for Gradients calculation for PDE residuals:
...
# Forward Pass:
	def forward(self, x):
		...
		return  torch.cat([x1,x2,x3],axis=1)  # concatenating the parallely processes outputs by CNN for U, V and P
